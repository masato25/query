...
import (
	...
	"github.com/masato25/query/gin_http/computeFunc"
...
func GetQueryTargets(c *gin.Context) {
	...
	var resRespCoustom []model.CostomGraphQueryResponse
	for _, target := range mtarget {
		...
		if ldfunction == "null" || len(result) == 0 {
			...
			json.Unmarshal([]byte(output.String()), &resRespCoustom)
			log.Debugf("outputStr: %v", output.String())
		}
	}
	if len(resRespCoustom) > 0 {
		c.JSON(200, resRespCoustom)
	} else {
		c.JSON(200, resResp)
	}
	return
...
func GetQueryTargetMuti(c *gin.Context) {
	var params GrafanaPostPrams
	err := c.Bind(&params)
	if err != nil {
		log.Error(err.Error())
	...
	for _, target := range mtarget {
		...
		log.Debugf("got endpoints : %d itmes", len(endpid))
		counter = strings.Replace(counter, "#", ".", -1)
		counters := strings.Split(counter, ",")
		startTs := params.From
		endTs := params.Until
		result := []*cmodel.GraphQueryResponse{}
		log.Debugf("got counter : %d itmes", len(counters))
